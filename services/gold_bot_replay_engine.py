...
def score_horizon(bars: list[HistoricalBar], index: int, *, decision: str, horizon: int,
                  sl_points: int | None, tp_points: int | None, point: float = POINT) -> dict:
...
def score_forward(bars, index, *, decision, sl_points, tp_points, horizons, point=POINT) -> dict:
...
class _Aggregator:
    def __init__(self, horizons: tuple[int, ...]):
        ...
        self.h: dict[str, dict] = {
            str(hz): {"win": 0, "loss": 0, "neutral": 0, "no_data": 0, "no_trade": 0,
                      "ret_sum": 0.0, "ret_count": 0} for hz in horizons}
    ...
    def add(self, idea, scores: dict) -> None:
        ...
        for hz, sc in scores.items():
            ...
            if idea.decision in ("LONG", "SHORT") and sc.get("dir_return_points") is not None:
                bucket["ret_sum"] += sc["dir_return_points"]
                bucket["ret_count"] += 1
